...
export default function CursosCarreiraPage() {
  ...
  const onSubmit = async (data: z.infer<typeof cursoSchema>) => {
    ...
    try {
      ...
      const cleanedData = {
        ...
        categoria: 'Carreira' // Force category
      };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t.courses.titleCarreira}</h1>
          <p className="text-slate-500 text-sm">Visualize e gerencie os cursos da categoria de Carreira.</p>
        </div>
        {!isReadOnly && (
